...
RelationDefinition.embedsOne = function(modelFrom, modelTo, params) {
  ...
  if (definition.options.validate !== false) {
    modelFrom.validate(relationName, function(err,done) {
      ...
      if (inst instanceof modelTo) {
        inst.isValid((function (valid) {
          if(!valid){
            ...
            console.log("Error object insertion not needed");
          }
  ...
  if (embeddedInstance instanceof modelTo) {
    ...
    modelTo.notifyObserversOf('before save', context, function(err) {
      ...
      embeddedInstance.isValid(function(valid) {
        console.log("made updation async");
        if(!valid){
          console.log("embedded instance invalid");
          return cb(new ValidationError(embeddedInstance));
        }
        modelInstance.updateAttribute(propertyName, embeddedInstance, function(err, inst) {
        if (err) return cb(err);
        ...
        modelTo.notifyObserversOf('after save', context, function(err) {
          console.log("saved embedded instance");
...
RelationDefinition.embedsMany = function embedsMany(modelFrom, modelTo, params) {
  ...
  if (typeof modelTo.dataSource.connector.generateId !== 'function') {
    modelFrom.validate(propertyName, function(err,done) {
      var self = this;
  ...
  if (definition.options.validate !== false) {
    modelFrom.validate(propertyName, function(err,done) {
      ...
      var embeddedList = this[propertyName] || [];
      var hasErrors = false;
      var embeddedListLength = embeddedList.length;
      var itemValidated = 0;
      if(embeddedListLength == 0){
        done();
      }
      embeddedList.forEach(function(item, idx) {
        if (item instanceof modelTo) {
          ...
          item.isValid(function(valid) {
            if(!valid) {
              ...
              if(!(self.errors instanceof Errors)){
              Object.defineProperty(self, 'errors', {
                    enumerable: false,
                    configurable: true,
                    value: new Errors,
               });
            ...
            itemValidated++;
            if(itemValidated == embeddedListLength){
              if(hasErrors){
                err(false);
              }
              done();
