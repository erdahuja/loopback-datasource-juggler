...
function List(items, itemType, parent, parseDatatype=true) {
  var list = this;
  if (!(list instanceof List)) {
    return new List(items, itemType, parent, parseDatatype);
  }
  ...
  items.forEach(function(item, i) {
    if (itemType && !(item instanceof itemType) && parseDatatype) {
      arr[i] = itemType(item);
    } else {
      arr[i] = item;
    }
