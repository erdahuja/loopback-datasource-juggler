...
Validatable.prototype.isValid = function(callback, data, options) {
  ...
  this.trigger('validate', function(validationsDone) {
    ...
    var modelRelations = inst.constructor.relations;
    var embedsRelationsList = {};
      for(var relationName in modelRelations){
        if(modelRelations[relationName]["embed"]){
          if(modelRelations[relationName]["type"] == "embedsOne"){
            embedsRelationsList[relationName] = true;
          }
          if(modelRelations[relationName]["type"] == "embedsMany"){
            embedsRelationsList[modelRelations[relationName]["keyFrom"]] = true;
          }
        }
      }
      console.log(embedsRelationsList);
    attrs.forEach(function(attr) {
      var attrValidations = validations[attr] || [];
      attrValidations.forEach(function(v) {
        if (v.options && (v.options.async || embedsRelationsList[attr])) {
          async = true;
          wait += 1;
          process.nextTick(function() {
