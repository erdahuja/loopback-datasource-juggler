...
var getNearFilterObject = (near,key,fullyQualifiedKey)=>{
  //if(!_.isEmpty(fullyQualifiedKey)){
   // key = `${fullyQualifiedKey}.${key}`;
   fullyQualifiedKey.push(key);
  //}
   return ({
          near: near.near,
...
var checkForNearFilter = (where,result,fullyQualifiedKey = [])=>{
  _.forEach(where,(value,key)=>{
      if((value && value.near) || result){
        result = result || getNearFilterObject(value,key,fullyQualifiedKey);
        return false;
      }
      if(_.isArray(value) || _.isObject(value)){
        //fullyQualifiedKey = _.isEmpty(fullyQualifiedKey) ? key : `${fullyQualifiedKey}.${key}`;
        fullyQualifiedKey.push(key);
        result = checkForNearFilter(value,result,fullyQualifiedKey);
      }
  })
    return result;
};
...
exports.nearFilter = function nearFilter(where) {
  ...
  //   });
  // }
  result = checkForNearFilter(where,result);
  return result;
};
