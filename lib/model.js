    ...
    if (!BASE_TYPES[type.name]) {
      ...
      if (type.prototype instanceof ModelBaseClass) {
        ...
        if (!(self.__data[p] instanceof List) &&
            ...
            self.__data[p] !== null) {
              let typename = Array.isArray(type) ? type[0].name : type.name,
              parseDatatype = typename !== 'ModelConstructor' ? false : true;
              self.__data[p] = List(self.__data[p], type, self, parseDatatype);
        }
      }
    }
  }
